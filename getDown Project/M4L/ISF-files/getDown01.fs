		...
		if(layer1enabled) {
			...
			if(i < elements1) {	
				...
				if(time1[i] > 0.0 && time1[i] < 0.25){
					...
					color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]*modT, stroke1[i]*modT);
				}
				...
				if(time1[i] > 0.25 && time1[i] < 0.5){
					...
					color1 += stroke(circleSDF(LOffset1[i],st01), scale1[i], modStroke*2.+.1-modScale);
					color1 += stroke(circleSDF(LOffset1[i],st01), scale1[i]-0.1, modStroke*2.+.1-modScale);
					color1 += stroke(circleSDF(LOffset1[i],st01), scale1[i]-0.2, modStroke*2.+.1-modScale);
					color1 += stroke(circleSDF(LOffset1[i],st01), scale1[i]-0.3, modStroke*2.+.1-modScale);
					
					
					LRot1[i] = LOffset1[i]+vec2(0.0,-0.1);
					
					vec2 tPos1 = rotate(0.5-st01+LOffset1[i],PI*1.+ang1[i])+vec2(0.0,-0.1);
					vec2 tPos2 = rotate(0.5-st01+LOffset1[i],PI*1.5+ang1[i])+vec2(0.0,-0.1);
					vec2 tPos3 = rotate(0.5-st01+LOffset1[i],PI*2.+ang1[i])+vec2(0.0,-0.1);
					vec2 tPos4 = rotate(0.5-st01+LOffset1[i],PI*2.5+ang1[i])+vec2(0.0,-0.1);
					
					
					color1 -= stroke(triSDF(tPos1), scale1[i]*modTinverse, modStroke+.12-modScale);				
					color1 -= stroke(triSDF(tPos2), scale1[i]*modTinverse, modStroke+.12-modScale);
					color1 -= stroke(triSDF(tPos3), scale1[i]*modTinverse, modStroke+.12-modScale);	
					color1 -= stroke(triSDF(tPos4), scale1[i]*modTinverse, modStroke+.12-modScale);	
				...
				if(time1[i] > 0.5 && time1[i] < 0.75){
					...
					float modStroke = stroke1[i]*2.;
					float modScale = modStroke-0.1;
				
					float modT = linearstep(0.5,0.75,time1[i]);
			
					color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i], stroke1[i]);
					color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]-0.1, stroke1[i]);
					color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]-0.2, stroke1[i]);
					color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]-0.3, stroke1[i]);	
					
					/*
					LRot1[i] = LOffset1[i]+vec2(0.0,-0.1);
					color1 -= stroke(triSDF(rotate(0.5-st01,PI*1.)+LRot1[i]), scale1[i]-0.12, stroke1[i]);
					color1 -= stroke(triSDF(rotate(0.5-st01,PI*1.5)+LRot1[i]), scale1[i]-0.12, stroke1[i]);	
					color1 -= stroke(triSDF(rotate(0.5-st01,PI*2.)+LRot1[i]), scale1[i]-0.12, stroke1[i]);	
					color1 -= stroke(triSDF(rotate(0.5-st01,PI*2.5)+LRot1[i]), scale1[i]-0.12, stroke1[i]);	
					*/
				}
				...
				if(time1[i] > 0.75 && time1[i] < 1.0){
					...
					color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]*modTinverse, stroke1[i]*modTinverse);
					color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]*modT, stroke1[i]*modTinverse);
					//color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]*modTinverse, stroke1[i]*modTinverse);
					//color1  += stroke(circleSDF(LOffset1[i],st01), scale1[i]*modTinverse, stroke1[i]*modTinverse);
		...
		if(layer2enabled){
			...
			if(i < elements2) {
				...
				if(time2[i] > 0.0 && time2[i] < 0.25){
					...
					LRot2[i] = rotate(0.5-st02+LOffset2[i], orient2[i]);
					color2  += stroke(rectSDF(LRot2[i],vec2(0.4)),scale2[i]*modT, stroke2[i]*modT);
					color2  += stroke(rectSDF(LRot2[i],vec2(0.4)),(scale2[i]-0.125)*modT, (stroke2[i]-0.01)*modT);
					color2  += stroke(rectSDF(LRot2[i],vec2(0.4)),(scale2[i]-0.25)*modT, (stroke2[i]-0.005)*modT);
					color2  += stroke(rectSDF(LRot2[i],vec2(0.4)),(scale2[i]-0.4)*modT, (stroke2[i]-0.02)*modT);
					
					color2  -= fill(rectSDF(rotate(LRot2[i],radians(45.)),vec2(1.0)),0.17*modT);
				}
		...
		if(layer3enabled){
			...
			if(i < elements3) {	
				...
				if(time3[i] > 0.0 && time3[i] < 0.25){
					...
					LRot3[i] = rotate(0.5-st03+LOffset3[i], orient3[i]);
		...
		if(layer4enabled){
			...
			if(i < elements4) {	
				...
				if(time4[i] > 0.0 && time4[i] < 0.25){
					...
					LRot4[i] = rotate(0.5-st04+LOffset4[i], orient4[i]);
